...
const DEFAULT_JOB_TYPES = ["Valve", "Pump", "Motor", "Electrical", "Mechanical", "Solenoid", "Seals"];
const DEFAULT_LOCATION_TAGS = ["Basement", "Gea Matrix", "Fruit", "Rice", "Buffer Tanks", "P1", "P2"];
const SAP = () => {
  ...
  const saveEditEntry = () => {
    ...
    setEditingEntries(editingEntries.map(entry => entry.id === editingEntryId ? {
      ...entry,
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter(job => {
      ...
      return (job.entries || []).some(entry => (entry.sapNumber || '').toLowerCase().includes(lowerSearch) || (entry.storeLocation || '').toLowerCase().includes(lowerSearch) || (entry.description || '').toLowerCase().includes(lowerSearch));
    });
    ...
    const updatedJobs = jobs.map(job => {
      if (job.id === jobId) {
        ...
        if (!currentTags.includes(tag.trim())) {
          return {
            ...job,
            locationTags: [...currentTags, tag.trim()]
          };
        }
      }
      return job;
      ...
      <div className="w-full mx-auto space-y-4 sm:space-y-6 px-2 sm:px-4">
            ...
            <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input value={jobName} onChange={e => setJobName(e.target.value)} placeholder="e.g., Common Maintenance Tasks" />
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    <div className="flex flex-wrap gap-2 p-3 border rounded-md min-h-[44px]">
                      {selectedJobTypes.length === 0 ? <span className="text-sm text-muted-foreground">No job types selected</span> : selectedJobTypes.map(type => <Badge key={type} variant="default" className="gap-1">
                            {type}
                            <button onClick={() => toggleJobType(type)} className="ml-1 hover:text-destructive">
                              <X className="h-3 w-3" />
                            </button>
                          </Badge>)}
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {jobTypes.map(type => <div key={type} className="relative group">
                          <Button type="button" variant={selectedJobTypes.includes(type) ? "secondary" : "outline"} size="sm" onClick={() => toggleJobType(type)} className="h-8 pr-2">
                            {type}
                          </Button>
                          {!DEFAULT_JOB_TYPES.includes(type) && <button type="button" onClick={e => {
                          e.stopPropagation();
                          removeJobType(type);
                        }} className="absolute -top-1 -right-1 h-4 w-4 rounded-full bg-destructive text-destructive-foreground hover:bg-destructive/90 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" title="Remove job type">
                              <X className="h-3 w-3" />
                            </button>}
                        </div>)}
                      {!isAddingJobType ? <Button type="button" variant="outline" size="sm" onClick={() => setIsAddingJobType(true)} className="h-8">
                          <Plus className="h-4 w-4 mr-1" />
                          Add Type
                        </Button> : <div className="flex gap-1">
                          <Input value={newJobType} onChange={e => setNewJobType(e.target.value)} placeholder="New type" className="h-8 w-32" onKeyDown={e => {
                          if (e.key === "Enter") {
                            e.preventDefault();
                            addJobType();
                          }
                ...
                <div>
                  ...
                  <div className="space-y-2 mt-2">
                    {editingEntries.map(entry => <div key={entry.id} className="flex items-start gap-2 p-3 bg-muted rounded-md">
                        {editingEntryId === entry.id ? <div className="flex-1 space-y-2">
                            <div className="space-y-1">
                              <label className="text-xs font-semibold text-muted-foreground">SAP:</label>
                              <Input value={editEntrySapNumber} onChange={e => setEditEntrySapNumber(e.target.value)} placeholder="SAP Number" className="h-8" />
                            </div>
                            <div className="space-y-1">
                              <label className="text-xs font-semibold text-muted-foreground">Location:</label>
                              <Input value={editEntryLocation} onChange={e => setEditEntryLocation(e.target.value)} placeholder="Store Location" className="h-8" />
                            </div>
                            <div className="space-y-1">
                              <label className="text-xs font-semibold text-muted-foreground">Description:</label>
                              <Textarea value={editEntryDescription} onChange={e => setEditEntryDescription(e.target.value)} placeholder="Description" rows={2} />
                            </div>
                            ...
                            <div className="flex gap-1">
                              <Button variant="ghost" size="icon" onClick={() => startEditEntry(entry)} className="h-8 w-8">
                                <Edit className="h-4 w-4" />
                              </Button>
                              <Button variant="ghost" size="icon" onClick={() => removeEntry(entry.id)} className="h-8 w-8 text-destructive hover:text-destructive">
                    ...
                    <div className="space-y-2 p-3 border rounded-md">
                      ...
                      <Input value={entrySapNumber} onChange={e => setEntrySapNumber(e.target.value)} placeholder="SAP Number" className="h-9" />
                      <Input value={entryLocation} onChange={e => setEntryLocation(e.target.value)} placeholder="Store Location" className="h-9" />
                      <Textarea value={entryDescription} onChange={e => setEntryDescription(e.target.value)} placeholder="Description" rows={2} />
        ...
        <div className="space-y-3">
          <div className="relative">
            ...
            <Input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search by job name, SAP number, location, description, or tags..." className="pl-10" />
          </div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Select value={selectedJobTypeFilter} onValueChange={setSelectedJobTypeFilter}>
              ...
              <SelectContent>
                ...
                {jobTypes.map(type => <SelectItem key={type} value={type}>
                    {type}
                  </SelectItem>)}
              </SelectContent>
            </Select>
            ...
            <Select value={selectedLocationFilter} onValueChange={setSelectedLocationFilter}>
              ...
              <SelectContent>
                ...
                {Array.from(new Set(jobs.flatMap(job => job.locationTags || []))).sort().map(tag => <SelectItem key={tag} value={tag}>
                    {tag}
                  </SelectItem>)}
              </SelectContent>
            </Select>
            
            {(selectedJobTypeFilter !== "all" || selectedLocationFilter !== "all") && <Button variant="ghost" size="sm" onClick={() => {
              ...
              <CardContent className="py-12">
                ...
                <CardHeader className="py-3 px-4">
                  <div className="flex items-start justify-between gap-4">
                      ...
                      <Button variant="ghost" size="icon" className="h-6 w-6 mt-0.5" onClick={() => toggleExpand(job.id)}>
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          {(expandedJobTypes.has(job.id) ? job.jobTypes : (job.jobTypes || []).slice(0, 3)).map(type => <Badge key={type} variant="outline" className="font-normal">
                              {type}
                            </Badge>)}
                          {!expandedJobTypes.has(job.id) && (job.jobTypes || []).length > 3 && <Button variant="ghost" size="sm" onClick={() => setExpandedJobTypes(prev => new Set(prev).add(job.id))} className="h-6 px-2 text-xs">
                              +{(job.jobTypes || []).length - 3} more
                            </Button>}
                          {expandedJobTypes.has(job.id) && (job.jobTypes || []).length > 3 && <Button variant="ghost" size="sm" onClick={() => {
                      ...
                      setExpandedJobTypes(newSet);
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {(expandedLocationTags.has(job.id) ? job.locationTags || [] : (job.locationTags || []).slice(0, 5)).map(tag => <Badge key={tag} variant="secondary" className="gap-1">
                              <Tag className="h-3 w-3" />
                              {tag}
                              <button onClick={() => removeLocationTag(job.id, tag)} className="ml-1 hover:text-destructive">
                                <X className="h-3 w-3" />
                              </button>
                            </Badge>)}
                          {!expandedLocationTags.has(job.id) && (job.locationTags || []).length > 5 && <Button variant="ghost" size="sm" onClick={() => setExpandedLocationTags(prev => new Set(prev).add(job.id))} className="h-6 px-2 text-xs">
                              +{(job.locationTags || []).length - 5} more
                            </Button>}
                          {expandedLocationTags.has(job.id) && (job.locationTags || []).length > 5 && <Button variant="ghost" size="sm" onClick={() => {
                      ...
                      if (!open) setNewLocationTag("");
                            ...
                            <PopoverTrigger asChild>
                              <Button variant="ghost" size="sm" className="h-6 px-2 text-xs" onClick={() => setAddingTagToJob(job.id)}>
                            ...
                            <PopoverContent className="w-80 bg-background" align="start">
                              <div className="space-y-3">
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {DEFAULT_LOCATION_TAGS.map(tag => <Button key={tag} type="button" variant="secondary" size="sm" onClick={() => addLocationTag(job.id, tag)} className="h-7 text-xs">
                                        {tag}
                                      </Button>)}
                                  </div>
                                </div>
                                ...
                                <div className="space-y-2">
                                  ...
                                  <div className="flex gap-2">
                                    <Input value={newLocationTag} onChange={e => setNewLocationTag(e.target.value)} placeholder="Custom location" className="h-8" onKeyDown={e => {
                                if (e.key === "Enter") {
                                  addLocationTag(job.id, newLocationTag);
                                }
                              }} />
                                    <Button size="sm" className="h-8" onClick={() => addLocationTag(job.id, newLocationTag)}>
                                      Add
                                    </Button>
                                  </div>
                                </div>
                    ...
                    <div className="flex gap-1">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => handleDeleteJob(job.id)} className="h-8 w-8 text-destructive hover:text-destructive">
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
                  </div>
                ...
                {job.expanded && (job.entries || []).length > 0 && <CardContent className="py-2 px-4 border-t">
                    <div className="space-y-2 ml-8">
                      {(job.entries || []).map((entry, index) => <div key={entry.id} className="text-sm py-2 px-3 bg-muted/50 rounded space-y-1">
                          <div className="flex items-center gap-2">
